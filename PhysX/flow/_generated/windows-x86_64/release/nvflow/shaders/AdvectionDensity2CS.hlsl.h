#include "AdvectionDensity2CS_vulkan.hlsl.h"
#include "AdvectionDensity2CS_d3d12.hlsl.h"

struct AdvectionDensity2CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void AdvectionDensity2CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, AdvectionDensity2CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = AdvectionDensity2CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(AdvectionDensity2CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 8;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = AdvectionDensity2CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(AdvectionDensity2CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void AdvectionDensity2CS_destroy(NvFlowContext* context, AdvectionDensity2CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct AdvectionDensity2CS_PassParams
{
...
inline void AdvectionDensity2CS_addPassCompute(NvFlowContext* context, AdvectionDensity2CS_Pipeline* ptr, NvFlowUint3 gridDim, const AdvectionDensity2CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[5].textureTransient = params->velocityIn;
		...
		resources[7].textureTransient = params->densityOut;
		...
		passComputeParams.debugLabel = "AdvectionDensity2CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
