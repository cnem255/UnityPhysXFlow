#include "SparseInplaceRescale4cCS_vulkan.hlsl.h"
#include "SparseInplaceRescale4cCS_d3d12.hlsl.h"

struct SparseInplaceRescale4cCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void SparseInplaceRescale4cCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, SparseInplaceRescale4cCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = SparseInplaceRescale4cCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseInplaceRescale4cCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 5;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = SparseInplaceRescale4cCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseInplaceRescale4cCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void SparseInplaceRescale4cCS_destroy(NvFlowContext* context, SparseInplaceRescale4cCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct SparseInplaceRescale4cCS_PassParams
{
...
inline void SparseInplaceRescale4cCS_addPassCompute(NvFlowContext* context, SparseInplaceRescale4cCS_Pipeline* ptr, NvFlowUint3 gridDim, const SparseInplaceRescale4cCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		passComputeParams.debugLabel = "SparseInplaceRescale4cCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
