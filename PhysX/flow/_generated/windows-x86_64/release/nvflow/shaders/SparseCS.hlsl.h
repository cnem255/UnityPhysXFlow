#include "SparseCS_vulkan.hlsl.h"
#include "SparseCS_d3d12.hlsl.h"

struct SparseCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void SparseCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, SparseCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = SparseCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 3;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = SparseCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void SparseCS_destroy(NvFlowContext* context, SparseCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct SparseCS_PassParams
{
	NvFlowBufferTransient* gParams;
	NvFlowBufferTransient* gTableIn;
	NvFlowBufferTransient* gTableOut;
};

inline void SparseCS_addPassCompute(NvFlowContext* context, SparseCS_Pipeline* ptr, NvFlowUint3 gridDim, const SparseCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[2].bufferTransient = params->gTableOut;
		...
		passComputeParams.debugLabel = "SparseCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
