#include "AdvectionVelocity2CS_vulkan.hlsl.h"
#include "AdvectionVelocity2CS_d3d12.hlsl.h"

struct AdvectionVelocity2CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void AdvectionVelocity2CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, AdvectionVelocity2CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = AdvectionVelocity2CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(AdvectionVelocity2CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 9;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = AdvectionVelocity2CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(AdvectionVelocity2CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void AdvectionVelocity2CS_destroy(NvFlowContext* context, AdvectionVelocity2CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct AdvectionVelocity2CS_PassParams
{
	...
	NvFlowTextureTransient* predictIn;
...
inline void AdvectionVelocity2CS_addPassCompute(NvFlowContext* context, AdvectionVelocity2CS_Pipeline* ptr, NvFlowUint3 gridDim, const AdvectionVelocity2CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[4].textureTransient = params->predictIn;
		...
		resources[6].sampler = params->valueSampler;
		...
		resources[7].textureTransient = params->velocityOut;
		...
		resources[8].textureTransient = params->densityCoarseOut;
		...
		passComputeParams.debugLabel = "AdvectionVelocity2CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
