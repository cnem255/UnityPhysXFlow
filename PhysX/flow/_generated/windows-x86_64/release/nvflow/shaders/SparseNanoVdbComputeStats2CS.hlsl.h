#include "SparseNanoVdbComputeStats2CS_vulkan.hlsl.h"
#include "SparseNanoVdbComputeStats2CS_d3d12.hlsl.h"

struct SparseNanoVdbComputeStats2CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void SparseNanoVdbComputeStats2CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, SparseNanoVdbComputeStats2CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = SparseNanoVdbComputeStats2CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseNanoVdbComputeStats2CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 3;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = SparseNanoVdbComputeStats2CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseNanoVdbComputeStats2CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void SparseNanoVdbComputeStats2CS_destroy(NvFlowContext* context, SparseNanoVdbComputeStats2CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct SparseNanoVdbComputeStats2CS_PassParams
{
...
inline void SparseNanoVdbComputeStats2CS_addPassCompute(NvFlowContext* context, SparseNanoVdbComputeStats2CS_Pipeline* ptr, NvFlowUint3 gridDim, const SparseNanoVdbComputeStats2CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		passComputeParams.debugLabel = "SparseNanoVdbComputeStats2CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
