#include "SparseClearNew4CS_vulkan.hlsl.h"
#include "SparseClearNew4CS_d3d12.hlsl.h"

struct SparseClearNew4CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void SparseClearNew4CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, SparseClearNew4CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = SparseClearNew4CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseClearNew4CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 3;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = SparseClearNew4CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseClearNew4CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void SparseClearNew4CS_destroy(NvFlowContext* context, SparseClearNew4CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct SparseClearNew4CS_PassParams
{
...
inline void SparseClearNew4CS_addPassCompute(NvFlowContext* context, SparseClearNew4CS_Pipeline* ptr, NvFlowUint3 gridDim, const SparseClearNew4CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		passComputeParams.debugLabel = "SparseClearNew4CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
