#include "SparseUploadCS_vulkan.hlsl.h"
#include "SparseUploadCS_d3d12.hlsl.h"

struct SparseUploadCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void SparseUploadCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, SparseUploadCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = SparseUploadCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseUploadCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 4;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = SparseUploadCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseUploadCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void SparseUploadCS_destroy(NvFlowContext* context, SparseUploadCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct SparseUploadCS_PassParams
{
	NvFlowBufferTransient* gParams;
	NvFlowBufferTransient* gTableIn;
	NvFlowBufferTransient* gTable1Out;
	NvFlowBufferTransient* gTable2Out;
};

inline void SparseUploadCS_addPassCompute(NvFlowContext* context, SparseUploadCS_Pipeline* ptr, NvFlowUint3 gridDim, const SparseUploadCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[1].bufferTransient = params->gTableIn;
		...
		resources[2].bufferTransient = params->gTable1Out;
		...
		resources[3].bufferTransient = params->gTable2Out;
		...
		passComputeParams.debugLabel = "SparseUploadCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
