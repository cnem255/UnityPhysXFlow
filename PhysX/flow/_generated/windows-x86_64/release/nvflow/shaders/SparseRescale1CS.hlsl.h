#include "SparseRescale1CS_vulkan.hlsl.h"
#include "SparseRescale1CS_d3d12.hlsl.h"

struct SparseRescale1CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void SparseRescale1CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, SparseRescale1CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = SparseRescale1CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseRescale1CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 7;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = SparseRescale1CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseRescale1CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void SparseRescale1CS_destroy(NvFlowContext* context, SparseRescale1CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct SparseRescale1CS_PassParams
{
...
inline void SparseRescale1CS_addPassCompute(NvFlowContext* context, SparseRescale1CS_Pipeline* ptr, NvFlowUint3 gridDim, const SparseRescale1CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		passComputeParams.debugLabel = "SparseRescale1CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
