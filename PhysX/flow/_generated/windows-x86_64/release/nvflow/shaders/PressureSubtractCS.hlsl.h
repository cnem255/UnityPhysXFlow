#include "PressureSubtractCS_vulkan.hlsl.h"
#include "PressureSubtractCS_d3d12.hlsl.h"

struct PressureSubtractCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void PressureSubtractCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, PressureSubtractCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = PressureSubtractCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(PressureSubtractCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 5;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = PressureSubtractCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(PressureSubtractCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void PressureSubtractCS_destroy(NvFlowContext* context, PressureSubtractCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct PressureSubtractCS_PassParams
{
...
inline void PressureSubtractCS_addPassCompute(NvFlowContext* context, PressureSubtractCS_Pipeline* ptr, NvFlowUint3 gridDim, const PressureSubtractCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		passComputeParams.debugLabel = "PressureSubtractCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
