#include "AdvectionDensity1CS_vulkan.hlsl.h"
#include "AdvectionDensity1CS_d3d12.hlsl.h"

struct AdvectionDensity1CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void AdvectionDensity1CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, AdvectionDensity1CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = AdvectionDensity1CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(AdvectionDensity1CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 7;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = AdvectionDensity1CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(AdvectionDensity1CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void AdvectionDensity1CS_destroy(NvFlowContext* context, AdvectionDensity1CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct AdvectionDensity1CS_PassParams
{
...
inline void AdvectionDensity1CS_addPassCompute(NvFlowContext* context, AdvectionDensity1CS_Pipeline* ptr, NvFlowUint3 gridDim, const AdvectionDensity1CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[5].sampler = params->valueSampler;
		...
		passComputeParams.debugLabel = "AdvectionDensity1CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
