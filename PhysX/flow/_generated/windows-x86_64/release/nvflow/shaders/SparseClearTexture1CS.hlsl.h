#include "SparseClearTexture1CS_vulkan.hlsl.h"
#include "SparseClearTexture1CS_d3d12.hlsl.h"

struct SparseClearTexture1CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void SparseClearTexture1CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, SparseClearTexture1CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = SparseClearTexture1CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseClearTexture1CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 2;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = SparseClearTexture1CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseClearTexture1CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void SparseClearTexture1CS_destroy(NvFlowContext* context, SparseClearTexture1CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct SparseClearTexture1CS_PassParams
{
	NvFlowBufferTransient* paramsIn;
	NvFlowTextureTransient* valueOut;
};

inline void SparseClearTexture1CS_addPassCompute(NvFlowContext* context, SparseClearTexture1CS_Pipeline* ptr, NvFlowUint3 gridDim, const SparseClearTexture1CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		passComputeParams.debugLabel = "SparseClearTexture1CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
