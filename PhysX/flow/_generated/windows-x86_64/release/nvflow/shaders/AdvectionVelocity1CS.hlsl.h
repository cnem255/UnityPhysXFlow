#include "AdvectionVelocity1CS_vulkan.hlsl.h"
#include "AdvectionVelocity1CS_d3d12.hlsl.h"

struct AdvectionVelocity1CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void AdvectionVelocity1CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, AdvectionVelocity1CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = AdvectionVelocity1CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(AdvectionVelocity1CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 6;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = AdvectionVelocity1CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(AdvectionVelocity1CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void AdvectionVelocity1CS_destroy(NvFlowContext* context, AdvectionVelocity1CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct AdvectionVelocity1CS_PassParams
{
...
inline void AdvectionVelocity1CS_addPassCompute(NvFlowContext* context, AdvectionVelocity1CS_Pipeline* ptr, NvFlowUint3 gridDim, const AdvectionVelocity1CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[5].textureTransient = params->velocityOut;
		...
		passComputeParams.debugLabel = "AdvectionVelocity1CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
