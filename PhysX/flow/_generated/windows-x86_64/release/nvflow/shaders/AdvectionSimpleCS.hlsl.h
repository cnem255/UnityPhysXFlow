#include "AdvectionSimpleCS_vulkan.hlsl.h"
#include "AdvectionSimpleCS_d3d12.hlsl.h"

struct AdvectionSimpleCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void AdvectionSimpleCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, AdvectionSimpleCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = AdvectionSimpleCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(AdvectionSimpleCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 6;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = AdvectionSimpleCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(AdvectionSimpleCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void AdvectionSimpleCS_destroy(NvFlowContext* context, AdvectionSimpleCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct AdvectionSimpleCS_PassParams
{
...
inline void AdvectionSimpleCS_addPassCompute(NvFlowContext* context, AdvectionSimpleCS_Pipeline* ptr, NvFlowUint3 gridDim, const AdvectionSimpleCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		passComputeParams.debugLabel = "AdvectionSimpleCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
