#include "AdvectionFadeVelocityCS_vulkan.hlsl.h"
#include "AdvectionFadeVelocityCS_d3d12.hlsl.h"

struct AdvectionFadeVelocityCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void AdvectionFadeVelocityCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, AdvectionFadeVelocityCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = AdvectionFadeVelocityCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(AdvectionFadeVelocityCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 5;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = AdvectionFadeVelocityCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(AdvectionFadeVelocityCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void AdvectionFadeVelocityCS_destroy(NvFlowContext* context, AdvectionFadeVelocityCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct AdvectionFadeVelocityCS_PassParams
{
...
inline void AdvectionFadeVelocityCS_addPassCompute(NvFlowContext* context, AdvectionFadeVelocityCS_Pipeline* ptr, NvFlowUint3 gridDim, const AdvectionFadeVelocityCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[4].textureTransient = params->velocityOut;
		...
		passComputeParams.debugLabel = "AdvectionFadeVelocityCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
