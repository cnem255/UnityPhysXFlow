#include "SparseNanoVdbPruneLeavesCS_vulkan.hlsl.h"
#include "SparseNanoVdbPruneLeavesCS_d3d12.hlsl.h"

struct SparseNanoVdbPruneLeavesCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void SparseNanoVdbPruneLeavesCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, SparseNanoVdbPruneLeavesCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = SparseNanoVdbPruneLeavesCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseNanoVdbPruneLeavesCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 4;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = SparseNanoVdbPruneLeavesCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseNanoVdbPruneLeavesCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void SparseNanoVdbPruneLeavesCS_destroy(NvFlowContext* context, SparseNanoVdbPruneLeavesCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct SparseNanoVdbPruneLeavesCS_PassParams
{
	NvFlowBufferTransient* paramsIn;
	NvFlowBufferTransient* cacheIn;
	NvFlowBufferTransient* nanovdbSrcIn;
	NvFlowBufferTransient* nanovdbInOut;
};

inline void SparseNanoVdbPruneLeavesCS_addPassCompute(NvFlowContext* context, SparseNanoVdbPruneLeavesCS_Pipeline* ptr, NvFlowUint3 gridDim, const SparseNanoVdbPruneLeavesCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[2].bufferTransient = params->nanovdbSrcIn;
		...
		resources[3].bufferTransient = params->nanovdbInOut;
		...
		passComputeParams.debugLabel = "SparseNanoVdbPruneLeavesCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
