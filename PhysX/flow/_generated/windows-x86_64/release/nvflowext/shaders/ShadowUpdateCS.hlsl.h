#include "ShadowUpdateCS_vulkan.hlsl.h"
#include "ShadowUpdateCS_d3d12.hlsl.h"

struct ShadowUpdateCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void ShadowUpdateCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, ShadowUpdateCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = ShadowUpdateCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(ShadowUpdateCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 8;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = ShadowUpdateCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(ShadowUpdateCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void ShadowUpdateCS_destroy(NvFlowContext* context, ShadowUpdateCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct ShadowUpdateCS_PassParams
{
...
inline void ShadowUpdateCS_addPassCompute(NvFlowContext* context, ShadowUpdateCS_Pipeline* ptr, NvFlowUint3 gridDim, const ShadowUpdateCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		passComputeParams.debugLabel = "ShadowUpdateCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
