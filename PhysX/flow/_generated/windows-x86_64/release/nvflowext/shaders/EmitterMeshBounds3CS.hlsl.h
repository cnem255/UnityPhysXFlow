#include "EmitterMeshBounds3CS_vulkan.hlsl.h"
#include "EmitterMeshBounds3CS_d3d12.hlsl.h"

struct EmitterMeshBounds3CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void EmitterMeshBounds3CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, EmitterMeshBounds3CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = EmitterMeshBounds3CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterMeshBounds3CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 3;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = EmitterMeshBounds3CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterMeshBounds3CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void EmitterMeshBounds3CS_destroy(NvFlowContext* context, EmitterMeshBounds3CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct EmitterMeshBounds3CS_PassParams
{
	NvFlowBufferTransient* gParams;
	NvFlowBufferTransient* bounds3In;
	NvFlowBufferTransient* bounds4Out;
};

inline void EmitterMeshBounds3CS_addPassCompute(NvFlowContext* context, EmitterMeshBounds3CS_Pipeline* ptr, NvFlowUint3 gridDim, const EmitterMeshBounds3CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[1].bufferTransient = params->bounds3In;
		...
		resources[2].bufferTransient = params->bounds4Out;
		...
		passComputeParams.debugLabel = "EmitterMeshBounds3CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
