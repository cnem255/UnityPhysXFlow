#include "EmitterMeshTree1CS_vulkan.hlsl.h"
#include "EmitterMeshTree1CS_d3d12.hlsl.h"

struct EmitterMeshTree1CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void EmitterMeshTree1CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, EmitterMeshTree1CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = EmitterMeshTree1CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterMeshTree1CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 7;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = EmitterMeshTree1CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterMeshTree1CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void EmitterMeshTree1CS_destroy(NvFlowContext* context, EmitterMeshTree1CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct EmitterMeshTree1CS_PassParams
{
	NvFlowBufferTransient* gParams;
	NvFlowBufferTransient* gSubStepParams;
	NvFlowBufferTransient* arrayValuesIn;
	NvFlowBufferTransient* sortKeyIn;
	NvFlowBufferTransient* sortValIn;
	NvFlowBufferTransient* bounds1Out;
	NvFlowBufferTransient* bounds2Out;
};

inline void EmitterMeshTree1CS_addPassCompute(NvFlowContext* context, EmitterMeshTree1CS_Pipeline* ptr, NvFlowUint3 gridDim, const EmitterMeshTree1CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[3].bufferTransient = params->sortKeyIn;
		...
		resources[5].bufferTransient = params->bounds1Out;
		...
		resources[6].bufferTransient = params->bounds2Out;
		...
		passComputeParams.debugLabel = "EmitterMeshTree1CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
