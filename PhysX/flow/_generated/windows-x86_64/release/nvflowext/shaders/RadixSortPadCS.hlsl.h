#include "RadixSortPadCS_vulkan.hlsl.h"
#include "RadixSortPadCS_d3d12.hlsl.h"

struct RadixSortPadCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void RadixSortPadCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, RadixSortPadCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = RadixSortPadCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(RadixSortPadCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 2;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = RadixSortPadCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(RadixSortPadCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void RadixSortPadCS_destroy(NvFlowContext* context, RadixSortPadCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct RadixSortPadCS_PassParams
{
	NvFlowBufferTransient* paramsIn;
	NvFlowBufferTransient* keyDst;
};

inline void RadixSortPadCS_addPassCompute(NvFlowContext* context, RadixSortPadCS_Pipeline* ptr, NvFlowUint3 gridDim, const RadixSortPadCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[1].bufferTransient = params->keyDst;
		...
		passComputeParams.debugLabel = "RadixSortPadCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
