#include "ShadowCoarseUpdateCS_vulkan.hlsl.h"
#include "ShadowCoarseUpdateCS_d3d12.hlsl.h"

struct ShadowCoarseUpdateCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void ShadowCoarseUpdateCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, ShadowCoarseUpdateCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = ShadowCoarseUpdateCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(ShadowCoarseUpdateCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 8;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = ShadowCoarseUpdateCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(ShadowCoarseUpdateCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void ShadowCoarseUpdateCS_destroy(NvFlowContext* context, ShadowCoarseUpdateCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct ShadowCoarseUpdateCS_PassParams
{
	...
	NvFlowTextureTransient* alphaOut;
};

inline void ShadowCoarseUpdateCS_addPassCompute(NvFlowContext* context, ShadowCoarseUpdateCS_Pipeline* ptr, NvFlowUint3 gridDim, const ShadowCoarseUpdateCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[7].textureTransient = params->alphaOut;
		...
		passComputeParams.debugLabel = "ShadowCoarseUpdateCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
