#include "EmitterNanoVdb2CS_vulkan.hlsl.h"
#include "EmitterNanoVdb2CS_d3d12.hlsl.h"

struct EmitterNanoVdb2CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void EmitterNanoVdb2CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, EmitterNanoVdb2CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = EmitterNanoVdb2CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterNanoVdb2CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 4;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = EmitterNanoVdb2CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterNanoVdb2CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void EmitterNanoVdb2CS_destroy(NvFlowContext* context, EmitterNanoVdb2CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct EmitterNanoVdb2CS_PassParams
{
...
inline void EmitterNanoVdb2CS_addPassCompute(NvFlowContext* context, EmitterNanoVdb2CS_Pipeline* ptr, NvFlowUint3 gridDim, const EmitterNanoVdb2CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		passComputeParams.debugLabel = "EmitterNanoVdb2CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
