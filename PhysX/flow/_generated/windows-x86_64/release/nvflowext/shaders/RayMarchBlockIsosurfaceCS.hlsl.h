#include "RayMarchBlockIsosurfaceCS_vulkan.hlsl.h"
#include "RayMarchBlockIsosurfaceCS_d3d12.hlsl.h"

struct RayMarchBlockIsosurfaceCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void RayMarchBlockIsosurfaceCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, RayMarchBlockIsosurfaceCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = RayMarchBlockIsosurfaceCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(RayMarchBlockIsosurfaceCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 8;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = RayMarchBlockIsosurfaceCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(RayMarchBlockIsosurfaceCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void RayMarchBlockIsosurfaceCS_destroy(NvFlowContext* context, RayMarchBlockIsosurfaceCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct RayMarchBlockIsosurfaceCS_PassParams
{
...
inline void RayMarchBlockIsosurfaceCS_addPassCompute(NvFlowContext* context, RayMarchBlockIsosurfaceCS_Pipeline* ptr, NvFlowUint3 gridDim, const RayMarchBlockIsosurfaceCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[4].sampler = params->densitySampler;
		...
		resources[5].textureTransient = params->depthIn;
		...
		resources[6].textureTransient = params->colorIn;
		...
		resources[7].textureTransient = params->colorOut;
		...
		passComputeParams.debugLabel = "RayMarchBlockIsosurfaceCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
