#include "DebugVolume2CS_vulkan.hlsl.h"
#include "DebugVolume2CS_d3d12.hlsl.h"

struct DebugVolume2CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void DebugVolume2CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, DebugVolume2CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = DebugVolume2CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(DebugVolume2CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 6;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = DebugVolume2CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(DebugVolume2CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void DebugVolume2CS_destroy(NvFlowContext* context, DebugVolume2CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct DebugVolume2CS_PassParams
{
	...
	NvFlowTextureTransient* coarseDensityShadowOut;
};

inline void DebugVolume2CS_addPassCompute(NvFlowContext* context, DebugVolume2CS_Pipeline* ptr, NvFlowUint3 gridDim, const DebugVolume2CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[4].sampler = params->samplerIn;
		...
		resources[5].textureTransient = params->coarseDensityShadowOut;
		...
		passComputeParams.debugLabel = "DebugVolume2CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
