#include "RayMarchCopyTextureCS_vulkan.hlsl.h"
#include "RayMarchCopyTextureCS_d3d12.hlsl.h"

struct RayMarchCopyTextureCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void RayMarchCopyTextureCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, RayMarchCopyTextureCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		bindingDescs[1].type = eNvFlowDescriptorType_texture;
		...
		desc.bytecode.data = RayMarchCopyTextureCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(RayMarchCopyTextureCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 3;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = RayMarchCopyTextureCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(RayMarchCopyTextureCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void RayMarchCopyTextureCS_destroy(NvFlowContext* context, RayMarchCopyTextureCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct RayMarchCopyTextureCS_PassParams
{
...
inline void RayMarchCopyTextureCS_addPassCompute(NvFlowContext* context, RayMarchCopyTextureCS_Pipeline* ptr, NvFlowUint3 gridDim, const RayMarchCopyTextureCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		descriptorWrites[1].type = eNvFlowDescriptorType_texture;
		...
		resources[1].textureTransient = params->colorIn;
		...
		resources[2].textureTransient = params->colorOut;
		...
		passComputeParams.debugLabel = "RayMarchCopyTextureCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
