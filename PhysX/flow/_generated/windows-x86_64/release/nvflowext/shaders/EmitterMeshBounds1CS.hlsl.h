#include "EmitterMeshBounds1CS_vulkan.hlsl.h"
#include "EmitterMeshBounds1CS_d3d12.hlsl.h"

struct EmitterMeshBounds1CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void EmitterMeshBounds1CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, EmitterMeshBounds1CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = EmitterMeshBounds1CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterMeshBounds1CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 4;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = EmitterMeshBounds1CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterMeshBounds1CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void EmitterMeshBounds1CS_destroy(NvFlowContext* context, EmitterMeshBounds1CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct EmitterMeshBounds1CS_PassParams
{
...
inline void EmitterMeshBounds1CS_addPassCompute(NvFlowContext* context, EmitterMeshBounds1CS_Pipeline* ptr, NvFlowUint3 gridDim, const EmitterMeshBounds1CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[3].bufferTransient = params->bounds2Out;
		...
		passComputeParams.debugLabel = "EmitterMeshBounds1CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
