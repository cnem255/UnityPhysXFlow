#include "EmitterPointClearMarkedCS_vulkan.hlsl.h"
#include "EmitterPointClearMarkedCS_d3d12.hlsl.h"

struct EmitterPointClearMarkedCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void EmitterPointClearMarkedCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, EmitterPointClearMarkedCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = EmitterPointClearMarkedCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterPointClearMarkedCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 4;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = EmitterPointClearMarkedCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterPointClearMarkedCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void EmitterPointClearMarkedCS_destroy(NvFlowContext* context, EmitterPointClearMarkedCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct EmitterPointClearMarkedCS_PassParams
{
...
inline void EmitterPointClearMarkedCS_addPassCompute(NvFlowContext* context, EmitterPointClearMarkedCS_Pipeline* ptr, NvFlowUint3 gridDim, const EmitterPointClearMarkedCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[2].textureTransient = params->voxelWeightIn;
		...
		passComputeParams.debugLabel = "EmitterPointClearMarkedCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
