#include "EmitterPointReduce2CS_vulkan.hlsl.h"
#include "EmitterPointReduce2CS_d3d12.hlsl.h"

struct EmitterPointReduce2CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void EmitterPointReduce2CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, EmitterPointReduce2CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = EmitterPointReduce2CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterPointReduce2CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 10;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = EmitterPointReduce2CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterPointReduce2CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void EmitterPointReduce2CS_destroy(NvFlowContext* context, EmitterPointReduce2CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct EmitterPointReduce2CS_PassParams
{
	...
	NvFlowBufferTransient* reduceTarget2Out;
	NvFlowBufferTransient* reduceCouple2Out;
	NvFlowBufferTransient* reduceWeight2Out;
	NvFlowBufferTransient* key2Out;
};

inline void EmitterPointReduce2CS_addPassCompute(NvFlowContext* context, EmitterPointReduce2CS_Pipeline* ptr, NvFlowUint3 gridDim, const EmitterPointReduce2CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[6].bufferTransient = params->reduceTarget2Out;
		...
		resources[7].bufferTransient = params->reduceCouple2Out;
		...
		resources[8].bufferTransient = params->reduceWeight2Out;
		...
		resources[9].bufferTransient = params->key2Out;
		...
		passComputeParams.debugLabel = "EmitterPointReduce2CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
