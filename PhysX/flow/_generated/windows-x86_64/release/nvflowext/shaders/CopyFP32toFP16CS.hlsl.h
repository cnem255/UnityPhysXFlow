#include "CopyFP32toFP16CS_vulkan.hlsl.h"
#include "CopyFP32toFP16CS_d3d12.hlsl.h"

struct CopyFP32toFP16CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void CopyFP32toFP16CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, CopyFP32toFP16CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = CopyFP32toFP16CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(CopyFP32toFP16CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 3;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = CopyFP32toFP16CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(CopyFP32toFP16CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void CopyFP32toFP16CS_destroy(NvFlowContext* context, CopyFP32toFP16CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct CopyFP32toFP16CS_PassParams
{
	NvFlowBufferTransient* paramsIn;
	NvFlowBufferTransient* srcIn;
	NvFlowBufferTransient* dstOut;
};

inline void CopyFP32toFP16CS_addPassCompute(NvFlowContext* context, CopyFP32toFP16CS_Pipeline* ptr, NvFlowUint3 gridDim, const CopyFP32toFP16CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[1].bufferTransient = params->srcIn;
		...
		resources[2].bufferTransient = params->dstOut;
		...
		passComputeParams.debugLabel = "CopyFP32toFP16CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
