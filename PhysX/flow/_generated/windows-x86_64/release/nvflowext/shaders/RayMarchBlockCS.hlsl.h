#include "RayMarchBlockCS_vulkan.hlsl.h"
#include "RayMarchBlockCS_d3d12.hlsl.h"

struct RayMarchBlockCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void RayMarchBlockCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, RayMarchBlockCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		bindingDescs[8].type = eNvFlowDescriptorType_sampler;
		...
		desc.bytecode.data = RayMarchBlockCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(RayMarchBlockCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		bindingDescs[8].type = eNvFlowDescriptorType_sampler;
		bindingDescs[8].bindingDesc.d3d12.registerHlsl = eNvFlowRegisterHlsl_s;
		...
		bindingDescs[9].bindingDesc.d3d12.registerBegin = 5;
		...
		bindingDescs[10].bindingDesc.d3d12.registerBegin = 6;
		...
		desc.numBindingDescs = 12;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = RayMarchBlockCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(RayMarchBlockCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void RayMarchBlockCS_destroy(NvFlowContext* context, RayMarchBlockCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct RayMarchBlockCS_PassParams
{
	...
	NvFlowSampler* velocitySampler;
	NvFlowTextureTransient* densityIn;
	NvFlowSampler* densitySampler;
	NvFlowTextureTransient* colormapIn;
	NvFlowSampler* colormapSampler;
	NvFlowTextureTransient* depthIn;
	NvFlowTextureTransient* colorIn;
	NvFlowTextureTransient* colorOut;
};

inline void RayMarchBlockCS_addPassCompute(NvFlowContext* context, RayMarchBlockCS_Pipeline* ptr, NvFlowUint3 gridDim, const RayMarchBlockCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[4].sampler = params->velocitySampler;
		...
		resources[5].textureTransient = params->densityIn;
		...
		resources[6].sampler = params->densitySampler;
		...
		resources[7].textureTransient = params->colormapIn;

		descriptorWrites[8].type = eNvFlowDescriptorType_sampler;
		...
		resources[8].sampler = params->colormapSampler;
		...
		resources[9].textureTransient = params->depthIn;
		...
		passComputeParams.debugLabel = "RayMarchBlockCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
		...
		descriptorWrites[8].type = eNvFlowDescriptorType_sampler;
		descriptorWrites[8].write.d3d12.registerHlsl = eNvFlowRegisterHlsl_s;
