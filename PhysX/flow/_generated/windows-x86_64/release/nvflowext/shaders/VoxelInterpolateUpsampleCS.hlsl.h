#include "VoxelInterpolateUpsampleCS_vulkan.hlsl.h"
#include "VoxelInterpolateUpsampleCS_d3d12.hlsl.h"

struct VoxelInterpolateUpsampleCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void VoxelInterpolateUpsampleCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, VoxelInterpolateUpsampleCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = VoxelInterpolateUpsampleCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(VoxelInterpolateUpsampleCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		bindingDescs[9].bindingDesc.d3d12.registerBegin = 1;
		bindingDescs[9].bindingDesc.d3d12.numRegisters = 1;
		bindingDescs[9].bindingDesc.d3d12.space = 0;

		desc.numBindingDescs = 10;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = VoxelInterpolateUpsampleCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(VoxelInterpolateUpsampleCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void VoxelInterpolateUpsampleCS_destroy(NvFlowContext* context, VoxelInterpolateUpsampleCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct VoxelInterpolateUpsampleCS_PassParams
{
	...
	NvFlowTextureTransient* coarseDensityIn;
	NvFlowTextureTransient* coarseVoxelWeightIn;
	NvFlowSampler* samplerIn;
	NvFlowTextureTransient* densityOut;
	NvFlowTextureTransient* voxelWeightOut;
};

inline void VoxelInterpolateUpsampleCS_addPassCompute(NvFlowContext* context, VoxelInterpolateUpsampleCS_Pipeline* ptr, NvFlowUint3 gridDim, const VoxelInterpolateUpsampleCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[5].textureTransient = params->coarseDensityIn;
		...
		resources[6].textureTransient = params->coarseVoxelWeightIn;
		...
		resources[9].textureTransient = params->voxelWeightOut;
		...
		passComputeParams.debugLabel = "VoxelInterpolateUpsampleCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
		...
		descriptorWrites[9].write.d3d12.registerIndex = 1;
