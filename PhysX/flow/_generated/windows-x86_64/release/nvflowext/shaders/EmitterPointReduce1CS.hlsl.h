#include "EmitterPointReduce1CS_vulkan.hlsl.h"
#include "EmitterPointReduce1CS_d3d12.hlsl.h"

struct EmitterPointReduce1CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void EmitterPointReduce1CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, EmitterPointReduce1CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = EmitterPointReduce1CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterPointReduce1CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		bindingDescs[8].bindingDesc.d3d12.registerBegin = 1;
		...
		bindingDescs[9].bindingDesc.d3d12.registerBegin = 2;
		...
		bindingDescs[10].bindingDesc.d3d12.registerBegin = 3;
		bindingDescs[10].bindingDesc.d3d12.numRegisters = 1;
		bindingDescs[10].bindingDesc.d3d12.space = 0;

		desc.numBindingDescs = 11;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = EmitterPointReduce1CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterPointReduce1CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void EmitterPointReduce1CS_destroy(NvFlowContext* context, EmitterPointReduce1CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct EmitterPointReduce1CS_PassParams
{
	...
	NvFlowBufferTransient* reduceTarget1Out;
	NvFlowBufferTransient* reduceCouple1Out;
	NvFlowBufferTransient* reduceWeight1Out;
	NvFlowBufferTransient* key1Out;
};

inline void EmitterPointReduce1CS_addPassCompute(NvFlowContext* context, EmitterPointReduce1CS_Pipeline* ptr, NvFlowUint3 gridDim, const EmitterPointReduce1CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[4].bufferTransient = params->keyLowIn;
		...
		resources[5].bufferTransient = params->keyIn;
		...
		resources[6].bufferTransient = params->valIn;
		...
		resources[7].bufferTransient = params->reduceTarget1Out;
		...
		resources[8].bufferTransient = params->reduceCouple1Out;
		...
		resources[9].bufferTransient = params->reduceWeight1Out;
		...
		resources[10].bufferTransient = params->key1Out;
		...
		passComputeParams.debugLabel = "EmitterPointReduce1CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
