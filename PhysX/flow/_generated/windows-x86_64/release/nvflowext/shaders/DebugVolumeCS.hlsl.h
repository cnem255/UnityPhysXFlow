#include "DebugVolumeCS_vulkan.hlsl.h"
#include "DebugVolumeCS_d3d12.hlsl.h"

struct DebugVolumeCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void DebugVolumeCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, DebugVolumeCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = DebugVolumeCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(DebugVolumeCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 7;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = DebugVolumeCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(DebugVolumeCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void DebugVolumeCS_destroy(NvFlowContext* context, DebugVolumeCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct DebugVolumeCS_PassParams
{
	NvFlowBufferTransient* globalParamsIn;
	NvFlowBufferTransient* layerParamsIn;
	NvFlowBufferTransient* tableIn;
	NvFlowTextureTransient* densityShadowIn;
	NvFlowTextureTransient* velocityIn;
	NvFlowSampler* samplerIn;
	NvFlowTextureTransient* densityShadowOut;
};

inline void DebugVolumeCS_addPassCompute(NvFlowContext* context, DebugVolumeCS_Pipeline* ptr, NvFlowUint3 gridDim, const DebugVolumeCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[3].textureTransient = params->densityShadowIn;
		...
		resources[4].textureTransient = params->velocityIn;
		...
		resources[6].textureTransient = params->densityShadowOut;
		...
		passComputeParams.debugLabel = "DebugVolumeCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
