#include "EmitterPointScan3CS_vulkan.hlsl.h"
#include "EmitterPointScan3CS_d3d12.hlsl.h"

struct EmitterPointScan3CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void EmitterPointScan3CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, EmitterPointScan3CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = EmitterPointScan3CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterPointScan3CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 9;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = EmitterPointScan3CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterPointScan3CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void EmitterPointScan3CS_destroy(NvFlowContext* context, EmitterPointScan3CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct EmitterPointScan3CS_PassParams
{
	NvFlowBufferTransient* gParams;
	NvFlowBufferTransient* gSubStepParams;
	NvFlowBufferTransient* reduceTarget3In;
	NvFlowBufferTransient* reduceCouple3In;
	NvFlowBufferTransient* reduceWeight3In;
	NvFlowBufferTransient* key3In;
	NvFlowBufferTransient* scanTarget3Out;
	NvFlowBufferTransient* scanCouple3Out;
	NvFlowBufferTransient* scanWeight3Out;
};

inline void EmitterPointScan3CS_addPassCompute(NvFlowContext* context, EmitterPointScan3CS_Pipeline* ptr, NvFlowUint3 gridDim, const EmitterPointScan3CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[2].bufferTransient = params->reduceTarget3In;
		...
		resources[3].bufferTransient = params->reduceCouple3In;
		...
		resources[4].bufferTransient = params->reduceWeight3In;
		...
		resources[5].bufferTransient = params->key3In;
		...
		resources[6].bufferTransient = params->scanTarget3Out;
		...
		resources[7].bufferTransient = params->scanCouple3Out;
		...
		resources[8].bufferTransient = params->scanWeight3Out;
		...
		passComputeParams.debugLabel = "EmitterPointScan3CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
