#include "EmitterPointScan2CS_vulkan.hlsl.h"
#include "EmitterPointScan2CS_d3d12.hlsl.h"

struct EmitterPointScan2CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void EmitterPointScan2CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, EmitterPointScan2CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = EmitterPointScan2CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterPointScan2CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 13;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = EmitterPointScan2CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterPointScan2CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void EmitterPointScan2CS_destroy(NvFlowContext* context, EmitterPointScan2CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct EmitterPointScan2CS_PassParams
{
	...
	NvFlowBufferTransient* scanTarget3In;
	NvFlowBufferTransient* scanCouple3In;
	NvFlowBufferTransient* scanWeight3In;
	NvFlowBufferTransient* key3In;
	NvFlowBufferTransient* scanTarget2Out;
	NvFlowBufferTransient* scanCouple2Out;
	NvFlowBufferTransient* scanWeight2Out;
};

inline void EmitterPointScan2CS_addPassCompute(NvFlowContext* context, EmitterPointScan2CS_Pipeline* ptr, NvFlowUint3 gridDim, const EmitterPointScan2CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[6].bufferTransient = params->scanTarget3In;
		...
		resources[7].bufferTransient = params->scanCouple3In;
		...
		resources[8].bufferTransient = params->scanWeight3In;
		...
		resources[9].bufferTransient = params->key3In;
		...
		resources[10].bufferTransient = params->scanTarget2Out;
		...
		resources[11].bufferTransient = params->scanCouple2Out;
		...
		resources[12].bufferTransient = params->scanWeight2Out;
		...
		passComputeParams.debugLabel = "EmitterPointScan2CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
