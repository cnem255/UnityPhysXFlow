#include "VoxelInterpolatePrepassCS_vulkan.hlsl.h"
#include "VoxelInterpolatePrepassCS_d3d12.hlsl.h"

struct VoxelInterpolatePrepassCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void VoxelInterpolatePrepassCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, VoxelInterpolatePrepassCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = VoxelInterpolatePrepassCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(VoxelInterpolatePrepassCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 7;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = VoxelInterpolatePrepassCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(VoxelInterpolatePrepassCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void VoxelInterpolatePrepassCS_destroy(NvFlowContext* context, VoxelInterpolatePrepassCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct VoxelInterpolatePrepassCS_PassParams
{
...
inline void VoxelInterpolatePrepassCS_addPassCompute(NvFlowContext* context, VoxelInterpolatePrepassCS_Pipeline* ptr, NvFlowUint3 gridDim, const VoxelInterpolatePrepassCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[5].textureTransient = params->densityOut;
		...
		resources[6].textureTransient = params->voxelWeightOut;
		...
		passComputeParams.debugLabel = "VoxelInterpolatePrepassCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
