#include "AdvectionFadeDensityCS_vulkan.hlsl.h"
#include "AdvectionFadeDensityCS_d3d12.hlsl.h"

struct AdvectionFadeDensityCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void AdvectionFadeDensityCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, AdvectionFadeDensityCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = AdvectionFadeDensityCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(AdvectionFadeDensityCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 5;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = AdvectionFadeDensityCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(AdvectionFadeDensityCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void AdvectionFadeDensityCS_destroy(NvFlowContext* context, AdvectionFadeDensityCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct AdvectionFadeDensityCS_PassParams
{
...
inline void AdvectionFadeDensityCS_addPassCompute(NvFlowContext* context, AdvectionFadeDensityCS_Pipeline* ptr, NvFlowUint3 gridDim, const AdvectionFadeDensityCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[4].textureTransient = params->densityOut;
		...
		passComputeParams.debugLabel = "AdvectionFadeDensityCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
