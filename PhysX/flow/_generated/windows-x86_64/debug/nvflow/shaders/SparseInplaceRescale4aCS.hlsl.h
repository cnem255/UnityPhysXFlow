#include "SparseInplaceRescale4aCS_vulkan.hlsl.h"
#include "SparseInplaceRescale4aCS_d3d12.hlsl.h"

struct SparseInplaceRescale4aCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void SparseInplaceRescale4aCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, SparseInplaceRescale4aCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = SparseInplaceRescale4aCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseInplaceRescale4aCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 6;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = SparseInplaceRescale4aCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseInplaceRescale4aCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void SparseInplaceRescale4aCS_destroy(NvFlowContext* context, SparseInplaceRescale4aCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct SparseInplaceRescale4aCS_PassParams
{
...
inline void SparseInplaceRescale4aCS_addPassCompute(NvFlowContext* context, SparseInplaceRescale4aCS_Pipeline* ptr, NvFlowUint3 gridDim, const SparseInplaceRescale4aCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[5].textureTransient = params->valueOut;
		...
		passComputeParams.debugLabel = "SparseInplaceRescale4aCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
