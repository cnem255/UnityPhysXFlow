#include "PressureProlongCS_vulkan.hlsl.h"
#include "PressureProlongCS_d3d12.hlsl.h"

struct PressureProlongCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void PressureProlongCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, PressureProlongCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = PressureProlongCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(PressureProlongCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 6;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = PressureProlongCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(PressureProlongCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void PressureProlongCS_destroy(NvFlowContext* context, PressureProlongCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct PressureProlongCS_PassParams
{
	NvFlowBufferTransient* gParams;
	NvFlowBufferTransient* gTable;
	NvFlowTextureTransient* pressureFineIn;
	NvFlowTextureTransient* pressureCoarseIn;
	NvFlowSampler* valueSampler;
	NvFlowTextureTransient* pressureFineOut;
};

inline void PressureProlongCS_addPassCompute(NvFlowContext* context, PressureProlongCS_Pipeline* ptr, NvFlowUint3 gridDim, const PressureProlongCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[2].textureTransient = params->pressureFineIn;
		...
		resources[3].textureTransient = params->pressureCoarseIn;
		...
		resources[5].textureTransient = params->pressureFineOut;
		...
		passComputeParams.debugLabel = "PressureProlongCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
