#include "SparseInplaceRescale4bCS_vulkan.hlsl.h"
#include "SparseInplaceRescale4bCS_d3d12.hlsl.h"

struct SparseInplaceRescale4bCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void SparseInplaceRescale4bCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, SparseInplaceRescale4bCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = SparseInplaceRescale4bCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseInplaceRescale4bCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 5;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = SparseInplaceRescale4bCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseInplaceRescale4bCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void SparseInplaceRescale4bCS_destroy(NvFlowContext* context, SparseInplaceRescale4bCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct SparseInplaceRescale4bCS_PassParams
{
...
inline void SparseInplaceRescale4bCS_addPassCompute(NvFlowContext* context, SparseInplaceRescale4bCS_Pipeline* ptr, NvFlowUint3 gridDim, const SparseInplaceRescale4bCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		passComputeParams.debugLabel = "SparseInplaceRescale4bCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
