#include "SparseRescale2CS_vulkan.hlsl.h"
#include "SparseRescale2CS_d3d12.hlsl.h"

struct SparseRescale2CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void SparseRescale2CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, SparseRescale2CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = SparseRescale2CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseRescale2CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 7;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = SparseRescale2CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(SparseRescale2CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void SparseRescale2CS_destroy(NvFlowContext* context, SparseRescale2CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct SparseRescale2CS_PassParams
{
...
inline void SparseRescale2CS_addPassCompute(NvFlowContext* context, SparseRescale2CS_Pipeline* ptr, NvFlowUint3 gridDim, const SparseRescale2CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		passComputeParams.debugLabel = "SparseRescale2CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
