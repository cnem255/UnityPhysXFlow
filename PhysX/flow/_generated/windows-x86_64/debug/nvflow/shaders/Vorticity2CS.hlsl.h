#include "Vorticity2CS_vulkan.hlsl.h"
#include "Vorticity2CS_d3d12.hlsl.h"

struct Vorticity2CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void Vorticity2CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, Vorticity2CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = Vorticity2CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(Vorticity2CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 8;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = Vorticity2CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(Vorticity2CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void Vorticity2CS_destroy(NvFlowContext* context, Vorticity2CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct Vorticity2CS_PassParams
{
	...
	NvFlowTextureTransient* curlIn;
	NvFlowTextureTransient* lowpassIn;
	NvFlowTextureTransient* coarseDensityIn;
	NvFlowTextureTransient* velocityOut;
};

inline void Vorticity2CS_addPassCompute(NvFlowContext* context, Vorticity2CS_Pipeline* ptr, NvFlowUint3 gridDim, const Vorticity2CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[4].textureTransient = params->curlIn;
		...
		resources[5].textureTransient = params->lowpassIn;
		...
		resources[6].textureTransient = params->coarseDensityIn;
		...
		passComputeParams.debugLabel = "Vorticity2CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
