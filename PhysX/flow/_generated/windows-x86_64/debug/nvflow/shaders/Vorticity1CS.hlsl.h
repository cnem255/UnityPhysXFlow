#include "Vorticity1CS_vulkan.hlsl.h"
#include "Vorticity1CS_d3d12.hlsl.h"

struct Vorticity1CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void Vorticity1CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, Vorticity1CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = Vorticity1CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(Vorticity1CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 6;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = Vorticity1CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(Vorticity1CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void Vorticity1CS_destroy(NvFlowContext* context, Vorticity1CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct Vorticity1CS_PassParams
{
	...
	NvFlowTextureTransient* curlOut;
	NvFlowTextureTransient* lowpassOut;
};

inline void Vorticity1CS_addPassCompute(NvFlowContext* context, Vorticity1CS_Pipeline* ptr, NvFlowUint3 gridDim, const Vorticity1CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[4].textureTransient = params->curlOut;
		...
		resources[5].textureTransient = params->lowpassOut;
		...
		passComputeParams.debugLabel = "Vorticity1CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
