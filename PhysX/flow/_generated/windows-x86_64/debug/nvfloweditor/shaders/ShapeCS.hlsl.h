#include "ShapeCS_vulkan.hlsl.h"
#include "ShapeCS_d3d12.hlsl.h"
...
inline void ShapeCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, ShapeCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = ShapeCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(ShapeCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 4;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = ShapeCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(ShapeCS_d3d12_hlsl);
	}
