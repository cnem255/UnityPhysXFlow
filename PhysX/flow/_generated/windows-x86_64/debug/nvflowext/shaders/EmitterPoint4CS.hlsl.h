#include "EmitterPoint4CS_vulkan.hlsl.h"
#include "EmitterPoint4CS_d3d12.hlsl.h"

struct EmitterPoint4CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void EmitterPoint4CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, EmitterPoint4CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = EmitterPoint4CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterPoint4CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 10;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = EmitterPoint4CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterPoint4CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void EmitterPoint4CS_destroy(NvFlowContext* context, EmitterPoint4CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct EmitterPoint4CS_PassParams
{
	...
	NvFlowTextureTransient* coarseValueOut;
};

inline void EmitterPoint4CS_addPassCompute(NvFlowContext* context, EmitterPoint4CS_Pipeline* ptr, NvFlowUint3 gridDim, const EmitterPoint4CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[4].bufferTransient = params->keyIn;
		...
		resources[5].bufferTransient = params->valIn;
		...
		resources[6].bufferTransient = params->keyLowIn;
		...
		resources[7].textureTransient = params->valueIn;
		...
		resources[8].sampler = params->valueSampler;
		...
		resources[9].textureTransient = params->coarseValueOut;
		...
		passComputeParams.debugLabel = "EmitterPoint4CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
