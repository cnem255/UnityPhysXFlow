#include "EmitterNanoVdbCS_vulkan.hlsl.h"
#include "EmitterNanoVdbCS_d3d12.hlsl.h"

struct EmitterNanoVdbCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void EmitterNanoVdbCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, EmitterNanoVdbCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = EmitterNanoVdbCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterNanoVdbCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		bindingDescs[9].bindingDesc.d3d12.registerBegin = 8;
		...
		bindingDescs[10].bindingDesc.d3d12.registerBegin = 9;
		...
		bindingDescs[12].bindingDesc.d3d12.registerBegin = 1;
		bindingDescs[12].bindingDesc.d3d12.numRegisters = 1;
		bindingDescs[12].bindingDesc.d3d12.space = 0;

		desc.numBindingDescs = 13;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = EmitterNanoVdbCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterNanoVdbCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void EmitterNanoVdbCS_destroy(NvFlowContext* context, EmitterNanoVdbCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct EmitterNanoVdbCS_PassParams
{
	NvFlowBufferTransient* gParams;
	NvFlowBufferTransient* gTable;
	NvFlowBufferTransient* arraysIn;
...
inline void EmitterNanoVdbCS_addPassCompute(NvFlowContext* context, EmitterNanoVdbCS_Pipeline* ptr, NvFlowUint3 gridDim, const EmitterNanoVdbCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[2].bufferTransient = params->arraysIn;
		...
		resources[3].bufferTransient = params->interopDistances;
		...
		resources[4].bufferTransient = params->interopVelocities;
		...
		resources[5].bufferTransient = params->interopDivergences;
		...
		resources[6].bufferTransient = params->interopTemperatures;
		...
		resources[7].bufferTransient = params->interopFuels;
		...
		resources[8].bufferTransient = params->interopBurns;
		...
		resources[9].bufferTransient = params->interopSmokes;
		...
		resources[10].textureTransient = params->valueIn;
		...
		resources[11].textureTransient = params->valueOut;
		...
		resources[12].textureTransient = params->voxelWeightOut;
		...
		passComputeParams.debugLabel = "EmitterNanoVdbCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
		...
		descriptorWrites[12].write.d3d12.registerIndex = 1;
