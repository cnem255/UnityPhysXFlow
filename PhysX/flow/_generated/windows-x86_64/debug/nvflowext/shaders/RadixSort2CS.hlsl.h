#include "RadixSort2CS_vulkan.hlsl.h"
#include "RadixSort2CS_d3d12.hlsl.h"

struct RadixSort2CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void RadixSort2CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, RadixSort2CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = RadixSort2CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(RadixSort2CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 3;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = RadixSort2CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(RadixSort2CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void RadixSort2CS_destroy(NvFlowContext* context, RadixSort2CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct RadixSort2CS_PassParams
{
	NvFlowBufferTransient* paramsIn;
	NvFlowBufferTransient* countersIn;
	NvFlowBufferTransient* countersOut;
};

inline void RadixSort2CS_addPassCompute(NvFlowContext* context, RadixSort2CS_Pipeline* ptr, NvFlowUint3 gridDim, const RadixSort2CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[1].bufferTransient = params->countersIn;
		...
		resources[2].bufferTransient = params->countersOut;
		...
		passComputeParams.debugLabel = "RadixSort2CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
