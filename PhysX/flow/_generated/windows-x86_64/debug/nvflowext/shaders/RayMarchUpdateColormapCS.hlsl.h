#include "RayMarchUpdateColormapCS_vulkan.hlsl.h"
#include "RayMarchUpdateColormapCS_d3d12.hlsl.h"

struct RayMarchUpdateColormapCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void RayMarchUpdateColormapCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, RayMarchUpdateColormapCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = RayMarchUpdateColormapCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(RayMarchUpdateColormapCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 4;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = RayMarchUpdateColormapCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(RayMarchUpdateColormapCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void RayMarchUpdateColormapCS_destroy(NvFlowContext* context, RayMarchUpdateColormapCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct RayMarchUpdateColormapCS_PassParams
{
	NvFlowBufferTransient* globalParamsIn;
	NvFlowBufferTransient* layerParamsIn;
	NvFlowBufferTransient* pointParamsIn;
	NvFlowTextureTransient* gColormapOut;
};

inline void RayMarchUpdateColormapCS_addPassCompute(NvFlowContext* context, RayMarchUpdateColormapCS_Pipeline* ptr, NvFlowUint3 gridDim, const RayMarchUpdateColormapCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[2].bufferTransient = params->pointParamsIn;
		...
		resources[3].textureTransient = params->gColormapOut;
		...
		passComputeParams.debugLabel = "RayMarchUpdateColormapCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
