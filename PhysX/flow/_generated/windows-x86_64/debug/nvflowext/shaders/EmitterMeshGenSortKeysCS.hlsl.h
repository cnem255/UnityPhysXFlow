#include "EmitterMeshGenSortKeysCS_vulkan.hlsl.h"
#include "EmitterMeshGenSortKeysCS_d3d12.hlsl.h"

struct EmitterMeshGenSortKeysCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void EmitterMeshGenSortKeysCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, EmitterMeshGenSortKeysCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = EmitterMeshGenSortKeysCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterMeshGenSortKeysCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 6;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = EmitterMeshGenSortKeysCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterMeshGenSortKeysCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void EmitterMeshGenSortKeysCS_destroy(NvFlowContext* context, EmitterMeshGenSortKeysCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct EmitterMeshGenSortKeysCS_PassParams
{
	...
	NvFlowBufferTransient* sortKeyOut;
	NvFlowBufferTransient* sortValOut;
};

inline void EmitterMeshGenSortKeysCS_addPassCompute(NvFlowContext* context, EmitterMeshGenSortKeysCS_Pipeline* ptr, NvFlowUint3 gridDim, const EmitterMeshGenSortKeysCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[2].bufferTransient = params->arrayValuesIn;
		...
		resources[3].bufferTransient = params->bounds4In;
		...
		resources[4].bufferTransient = params->sortKeyOut;
		...
		resources[5].bufferTransient = params->sortValOut;
		...
		passComputeParams.debugLabel = "EmitterMeshGenSortKeysCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
