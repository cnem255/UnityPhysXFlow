#include "EmitterTextureCS_vulkan.hlsl.h"
#include "EmitterTextureCS_d3d12.hlsl.h"

struct EmitterTextureCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void EmitterTextureCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, EmitterTextureCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = EmitterTextureCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterTextureCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 5;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = EmitterTextureCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterTextureCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void EmitterTextureCS_destroy(NvFlowContext* context, EmitterTextureCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct EmitterTextureCS_PassParams
{
...
inline void EmitterTextureCS_addPassCompute(NvFlowContext* context, EmitterTextureCS_Pipeline* ptr, NvFlowUint3 gridDim, const EmitterTextureCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		passComputeParams.debugLabel = "EmitterTextureCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
