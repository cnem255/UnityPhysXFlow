#include "VoxelInterpolatePropagateCS_vulkan.hlsl.h"
#include "VoxelInterpolatePropagateCS_d3d12.hlsl.h"

struct VoxelInterpolatePropagateCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void VoxelInterpolatePropagateCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, VoxelInterpolatePropagateCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = VoxelInterpolatePropagateCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(VoxelInterpolatePropagateCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 8;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = VoxelInterpolatePropagateCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(VoxelInterpolatePropagateCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void VoxelInterpolatePropagateCS_destroy(NvFlowContext* context, VoxelInterpolatePropagateCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct VoxelInterpolatePropagateCS_PassParams
{
...
inline void VoxelInterpolatePropagateCS_addPassCompute(NvFlowContext* context, VoxelInterpolatePropagateCS_Pipeline* ptr, NvFlowUint3 gridDim, const VoxelInterpolatePropagateCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		passComputeParams.debugLabel = "VoxelInterpolatePropagateCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
