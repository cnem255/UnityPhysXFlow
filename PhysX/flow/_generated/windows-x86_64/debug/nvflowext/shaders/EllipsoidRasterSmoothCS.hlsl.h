#include "EllipsoidRasterSmoothCS_vulkan.hlsl.h"
#include "EllipsoidRasterSmoothCS_d3d12.hlsl.h"

struct EllipsoidRasterSmoothCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void EllipsoidRasterSmoothCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, EllipsoidRasterSmoothCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = EllipsoidRasterSmoothCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EllipsoidRasterSmoothCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 7;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = EllipsoidRasterSmoothCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EllipsoidRasterSmoothCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void EllipsoidRasterSmoothCS_destroy(NvFlowContext* context, EllipsoidRasterSmoothCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct EllipsoidRasterSmoothCS_PassParams
{
...
inline void EllipsoidRasterSmoothCS_addPassCompute(NvFlowContext* context, EllipsoidRasterSmoothCS_Pipeline* ptr, NvFlowUint3 gridDim, const EllipsoidRasterSmoothCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[2].bufferTransient = params->blockIdxIn;
		...
		resources[3].bufferTransient = params->rangeIn;
		...
		resources[4].bufferTransient = params->instanceIdIn;
		...
		passComputeParams.debugLabel = "EllipsoidRasterSmoothCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
