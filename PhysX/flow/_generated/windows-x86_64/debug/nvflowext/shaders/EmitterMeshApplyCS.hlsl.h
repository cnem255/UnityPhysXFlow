#include "EmitterMeshApplyCS_vulkan.hlsl.h"
#include "EmitterMeshApplyCS_d3d12.hlsl.h"

struct EmitterMeshApplyCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void EmitterMeshApplyCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, EmitterMeshApplyCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = EmitterMeshApplyCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterMeshApplyCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 7;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = EmitterMeshApplyCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterMeshApplyCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void EmitterMeshApplyCS_destroy(NvFlowContext* context, EmitterMeshApplyCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct EmitterMeshApplyCS_PassParams
{
	...
	NvFlowTextureTransient* closestIn;
	NvFlowTextureTransient* valueOut;
};

inline void EmitterMeshApplyCS_addPassCompute(NvFlowContext* context, EmitterMeshApplyCS_Pipeline* ptr, NvFlowUint3 gridDim, const EmitterMeshApplyCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[5].textureTransient = params->closestIn;
		...
		passComputeParams.debugLabel = "EmitterMeshApplyCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
