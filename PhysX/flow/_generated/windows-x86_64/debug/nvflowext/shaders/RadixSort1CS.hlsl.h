#include "RadixSort1CS_vulkan.hlsl.h"
#include "RadixSort1CS_d3d12.hlsl.h"

struct RadixSort1CS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void RadixSort1CS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, RadixSort1CS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = RadixSort1CS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(RadixSort1CS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 3;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = RadixSort1CS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(RadixSort1CS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void RadixSort1CS_destroy(NvFlowContext* context, RadixSort1CS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct RadixSort1CS_PassParams
{
...
inline void RadixSort1CS_addPassCompute(NvFlowContext* context, RadixSort1CS_Pipeline* ptr, NvFlowUint3 gridDim, const RadixSort1CS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		passComputeParams.debugLabel = "RadixSort1CS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
