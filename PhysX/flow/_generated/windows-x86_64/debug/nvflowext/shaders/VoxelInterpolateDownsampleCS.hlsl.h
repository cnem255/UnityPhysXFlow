#include "VoxelInterpolateDownsampleCS_vulkan.hlsl.h"
#include "VoxelInterpolateDownsampleCS_d3d12.hlsl.h"

struct VoxelInterpolateDownsampleCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void VoxelInterpolateDownsampleCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, VoxelInterpolateDownsampleCS_Pipeline* ptr)
{
	ptr->contextInterface = contextInterface;
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	NvFlowBindingDesc bindingDescs[8] = {};
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		bindingDescs[5].type = eNvFlowDescriptorType_sampler;
		...
		desc.numBindingDescs = 8;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = VoxelInterpolateDownsampleCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(VoxelInterpolateDownsampleCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		bindingDescs[5].type = eNvFlowDescriptorType_sampler;
		bindingDescs[5].bindingDesc.d3d12.registerHlsl = eNvFlowRegisterHlsl_s;
		...
		desc.numBindingDescs = 8;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = VoxelInterpolateDownsampleCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(VoxelInterpolateDownsampleCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void VoxelInterpolateDownsampleCS_destroy(NvFlowContext* context, VoxelInterpolateDownsampleCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct VoxelInterpolateDownsampleCS_PassParams
{
	...
	NvFlowTextureTransient* voxelWeightIn;
	NvFlowSampler* samplerIn;
	NvFlowTextureTransient* densityOut;
	NvFlowTextureTransient* voxelWeightOut;
};

inline void VoxelInterpolateDownsampleCS_addPassCompute(NvFlowContext* context, VoxelInterpolateDownsampleCS_Pipeline* ptr, NvFlowUint3 gridDim, const VoxelInterpolateDownsampleCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	NvFlowDescriptorWrite descriptorWrites[8] = {};
	NvFlowResource resources[8] = {};
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[4].textureTransient = params->voxelWeightIn;

		descriptorWrites[5].type = eNvFlowDescriptorType_sampler;
		...
		resources[5].sampler = params->samplerIn;
		...
		resources[6].textureTransient = params->densityOut;
		...
		resources[7].textureTransient = params->voxelWeightOut;

		passComputeParams.numDescriptorWrites = 8;
		passComputeParams.descriptorWrites = descriptorWrites;
		passComputeParams.resources = resources;
		passComputeParams.debugLabel = "VoxelInterpolateDownsampleCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
		...
		descriptorWrites[5].type = eNvFlowDescriptorType_sampler;
		descriptorWrites[5].write.d3d12.registerHlsl = eNvFlowRegisterHlsl_s;
