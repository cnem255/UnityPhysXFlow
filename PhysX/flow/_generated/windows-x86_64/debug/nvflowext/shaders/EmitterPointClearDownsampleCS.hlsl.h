#include "EmitterPointClearDownsampleCS_vulkan.hlsl.h"
#include "EmitterPointClearDownsampleCS_d3d12.hlsl.h"

struct EmitterPointClearDownsampleCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void EmitterPointClearDownsampleCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, EmitterPointClearDownsampleCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = EmitterPointClearDownsampleCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterPointClearDownsampleCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 5;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = EmitterPointClearDownsampleCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterPointClearDownsampleCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void EmitterPointClearDownsampleCS_destroy(NvFlowContext* context, EmitterPointClearDownsampleCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct EmitterPointClearDownsampleCS_PassParams
{
...
inline void EmitterPointClearDownsampleCS_addPassCompute(NvFlowContext* context, EmitterPointClearDownsampleCS_Pipeline* ptr, NvFlowUint3 gridDim, const EmitterPointClearDownsampleCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[2].textureTransient = params->valueIn;
		...
		passComputeParams.debugLabel = "EmitterPointClearDownsampleCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
