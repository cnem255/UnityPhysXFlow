#include "EllipsoidRasterCS_vulkan.hlsl.h"
#include "EllipsoidRasterCS_d3d12.hlsl.h"

struct EllipsoidRasterCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void EllipsoidRasterCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, EllipsoidRasterCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = EllipsoidRasterCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EllipsoidRasterCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 9;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = EllipsoidRasterCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EllipsoidRasterCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void EllipsoidRasterCS_destroy(NvFlowContext* context, EllipsoidRasterCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct EllipsoidRasterCS_PassParams
{
	NvFlowBufferTransient* globalParamsIn;
	NvFlowBufferTransient* layerParamsIn;
	NvFlowBufferTransient* tableIn;
	NvFlowBufferTransient* arrayValuesIn1;
	NvFlowBufferTransient* arrayValuesIn4;
	NvFlowBufferTransient* blockIdxIn;
	NvFlowBufferTransient* rangeIn;
	NvFlowBufferTransient* instanceIdIn;
	NvFlowTextureTransient* valueOut;
};

inline void EllipsoidRasterCS_addPassCompute(NvFlowContext* context, EllipsoidRasterCS_Pipeline* ptr, NvFlowUint3 gridDim, const EllipsoidRasterCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		resources[3].bufferTransient = params->arrayValuesIn1;
		...
		resources[4].bufferTransient = params->arrayValuesIn4;
		...
		resources[5].bufferTransient = params->blockIdxIn;
		...
		resources[6].bufferTransient = params->rangeIn;
		...
		resources[7].bufferTransient = params->instanceIdIn;
		...
		resources[8].textureTransient = params->valueOut;
		...
		passComputeParams.debugLabel = "EllipsoidRasterCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
