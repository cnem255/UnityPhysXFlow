#include "EmitterSimpleCS_vulkan.hlsl.h"
#include "EmitterSimpleCS_d3d12.hlsl.h"

struct EmitterSimpleCS_Pipeline
{
	NvFlowContextInterface* contextInterface;
	NvFlowComputePipeline* pipeline;
};

inline void EmitterSimpleCS_init(NvFlowContextInterface* contextInterface, NvFlowContext* context, EmitterSimpleCS_Pipeline* ptr)
{
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		desc.bytecode.data = EmitterSimpleCS_vulkan_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterSimpleCS_vulkan_hlsl);
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		...
		desc.numBindingDescs = 6;
		desc.bindingDescs = bindingDescs;
		desc.bytecode.data = EmitterSimpleCS_d3d12_hlsl;
		desc.bytecode.sizeInBytes = sizeof(EmitterSimpleCS_d3d12_hlsl);
	}
	ptr->pipeline = ptr->contextInterface->createComputePipeline(context, &desc);
};

inline void EmitterSimpleCS_destroy(NvFlowContext* context, EmitterSimpleCS_Pipeline* ptr)
{
	ptr->contextInterface->destroyComputePipeline(context, ptr->pipeline);
}

struct EmitterSimpleCS_PassParams
{
...
inline void EmitterSimpleCS_addPassCompute(NvFlowContext* context, EmitterSimpleCS_Pipeline* ptr, NvFlowUint3 gridDim, const EmitterSimpleCS_PassParams* params)
{
	NvFlowContextConfig config = {};
	ptr->contextInterface->getContextConfig(context, &config);
	...
	if (config.api == eNvFlowContextApi_vulkan)
	{
		...
		passComputeParams.debugLabel = "EmitterSimpleCS";
	}
	if (config.api == eNvFlowContextApi_d3d12)
	{
		descriptorWrites[0].type = eNvFlowDescriptorType_constantBuffer;
